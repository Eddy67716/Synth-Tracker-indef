...
public class InstrumentController extends GenericController {
    ...
    public void midiProgramOnChange() {
        ...
        if (isAlteringDataSources()) {
            ...
            // update instrument global volume value
            selectedInstrument.setMidiProgram((byte) ((int) instrumentSpinner.getValue() - 1));
        }
    ...
    public void programTextChange(JSpinner instrumentSpinner) {
        ...
        int channelValue = (int) instrumentUI.getTools().getMidiOptions()
                .getMidiChannelSpinner().getValue();
    ...
    public void instrumentNameChange() {
        ...
        JLabel midiInstrumentNameLabel = instrumentUI.getTools()
                .getMidiOptions().getMidiInstrumentDisplayLabel();

        int bank = (int) instrumentUI.getTools()
                .getMidiOptions().getMidiBankSpinner().getValue();

        int program = (int) instrumentUI.getTools()
                .getMidiOptions().getMidiInstrumentSpinner().getValue();

        int channel = (int) instrumentUI.getTools().getMidiOptions()
                .getMidiChannelSpinner().getValue();
    ...
    public void instrumentOnChange() {
        ...
        if (selectedInstrument.getMidiProgram() >= -1
                && selectedInstrument.getMidiProgram() <= 127) {
            instrumentSpinner.setValue((int) selectedInstrument
                    .getMidiProgram() + 1);
        } else {
            instrumentSpinner.setValue(0);
        }
